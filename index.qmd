...
title-block-banner: "#145a9b"
title-block-style: default
date: "2026-04-27"
author:
  - name: "James Rix"
    affiliations:
      - name: "University of Exeter- Department of Economics"
format: html
css: formatting.css
toc: true
---
...
In this report a 25 year window will be used, taking data from 2001/01/01 - 2025/12/31, totalling 300 records. 
...
$$
\color{gray}{
\sigma_t = \sqrt{\frac{1}{n-1} \sum_{i=0}^{n-1} (\pi_{t-i} - \bar{\pi}_t)^2}, \quad n = 12
}
$$
...
<button onclick="showYearly()">Yearly</button>

<div id="monthly">
  <img src="Outputs/Figures/combined_timeseries_monthly.png" width="100%">
  <p><em><strong>Figure 1A.</strong> Monthly time series of inflation metrics and S&P500 returns and volatility.</em></p>
</div>

<div id="yearly" style="display:none;">
  <img src="Outputs/Figures/combined_timeseries_yearly.png" width="100%">
  <p><em><strong>Figure 1B.</strong> Yearly time series of inflation metrics and S&P500 returns and volatility.</em></p>
</div>
...
**Note**: *Click the "Monthly" and "Yearly" buttons to toggle between the respective data frequency.*
...
To begin investigating the relationship between inflation metrics and S&P500 returns and volatility, scatter plots can be used to provide a visual representation and an intuitive preview of what later regressions may uncover. To aid in the visual interpretation a line of best fit has been plotted on each graph. A tightly bound trend around the line of best fit hints at a strong positive or negative relationship, conversely a cloud of points with no clearly visible pattern may point at a weak or absent correlation.  

On visual inspection of *Figure 2*, there seems to be extremely little correlation between each inflation metric and S&P500 returns. R squared values are extremely small hinting that the relationship between inflation metrics and S&P500 returns is extremely week. Running a simple linear regression yields extremely high p-values, meaning any observable correlation is statistically insignificant.
...
![](Outputs/Figures/sp500_r_inflation_s.png){fig-alt="S&P500 Returns Against Inflation Surprise"}
:::
...
![](Outputs/Figures/sp500_v_inflation_s.png){fig-alt="S&P500 Volatility Against Inflation Surprise"}
:::
...
After inspecting *Figure 2* and *Figure 3*, there appears to be heteroskedasticity, thus robust standard errors should be used in later regressions. Additionally, there is visible clustering around the inflation level of 2-3%, inflation volatility from 0-1% and inflation surprise of -2-1PP. Economically these patterns make sense, given the FED's inflation target of 2% and inflation surprise being predominantly negative due to a multitude of economic factors. Furthermore, the plots of inflation vs S&P500 volatility and inflation surprise vs S&P500 volatility appear to follow a nonlinear relationship, this this should also be tested.
...
The results remain largely unchanged, with all metrics remaining statistically insignificant and adjusted R-squared only increasing to 0.011, revealing past inflation conditions do not predict future S&P500 returns.

We now turn to investigate the relationship between the inflation metrics and S&P500 volatility. As noted earlier, visual inspection of *Figure 3A* and *Figure 3B* suggests a non-linear relationship of inflation and inflation surprise with S&P500 volatility. To account for this squared terms for each of these respective metrics have been included in the model: 
...
The inclusion of past S&P500 volatility in the model produces an interesting result. As seen in Figure 7, the model yields a very high R-squared (0.938), yet the relationship of inflation, inflation volatility and inflation surprise to S&P500 volatility all become statistically insignificant (p-values: 0.433, 0.128, 0.408 respectively). 
...
The results strongly support the economic phenomena of volatility persistence and volatility clustering. It suggests that past volatility is the strongest predictor of future volatility and once accounted for, the inflation metrics provide almost no explanatory power. This suggests that the significant results for inflation metrics discovered in previous models, were likely due to omitting volatility persistence rather than reflecting a meaningful relationship.  

## Limitations

While this project provides a useful insight into the relationship between inflation metrics and S&P500 behaviour, there are several limitations to consider:

- **Data is sampled monthly**: Using monthly observations may obscure short term market reactions that happen on a daily basis.
- **Ommitted variable bias**: For feasability, other macroeconomic variables such as interest rates, GDP growth, unemployment etc. have not been included, so coefficients may be biased.
...
- *Federal Reserve Bank of St. Louis (FRED). CPIAUCSL and MICH datasets. Retrieved via the FRED API.*
- *Yahoo Finance. S&P 500 Index (^GSPC) historical price data.*
